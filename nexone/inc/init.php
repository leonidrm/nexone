...
class Nex_Theme_Includes
{
	...
	public static function init()
	{
		...
		$theme = wp_get_theme();
		define('NEX_SUBTHEME_NAME', $theme->get( 'Name' ));
		define('NEX_SUBTHEME_SLUG', sanitize_title( wp_trim_words( $theme->get( 'Name' ), 1 )));
		define('NEX_THEME_SLUG', 'nex');
	...
	public static function _action_widgets_init()
	{
		...
		foreach ($paths as $path) {
			...
			foreach ($dirs as $dir) {
				...
				self::include_isolated($dir .'/class-'.NEX_THEME_SLUG.'-widget-'. $dirname .'.php');

				$widget_class = NEX_THEME_SLUG . '_Widget_' . self::dirname_to_classname( $dirname );
